...
def g_info_loss(feat, model, j_actions, fake):

    q_feat = F.relu(model.gq_features(feat))
    action_logits = model.gq_action(q_feat)

    return (fake * action_information_loss(action_logits, j_actions)).mean()
...
def reshape_dist(dist, n_categ, n_classes):
    return dist.get_dist(dist.deter.shape[:-1], n_categ, n_classes)


def state_divergence_loss(prior, posterior, n_categ, n_classes, reduce=True, balance=0.2):
    prior_dist = reshape_dist(prior, n_categ, n_classes)
    post_dist = reshape_dist(posterior, n_categ, n_classes)
