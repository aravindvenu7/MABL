...
class DreamerLearner:
    ...
    def __init__(self, config):
        ...
        self.actor = Actor(config.FEAT, config.ACTION_SIZE, config.ACTION_HIDDEN, config.ACTION_LAYERS).to(
            config.DEVICE)
        if(config.GLOBAL):    
          self.critic = MADDPGCritic(config.GLOBAL_FEAT+config.FEAT, config.HIDDEN, global_feat = config.GLOBAL).to(config.DEVICE)
        else:  
          self.critic = MADDPGCritic(config.FEAT, config.HIDDEN, global_feat = config.GLOBAL).to(config.DEVICE)
        ...
        self.replay_buffer = DreamerMemory(config.CAPACITY, config.SEQ_LENGTH, config.ACTION_SIZE, config.IN_DIM, config.STATE_DIM, 2,
    ...
    def step(self, rollout):
        ...
        self.replay_buffer.append(rollout['observation'], rollout['global_state'], rollout['action'], rollout['reward'], rollout['done'],
    ...
    def train_model(self, samples):
        self.model.train()
        loss = model_loss(self.config, self.model, samples['observation'], samples['global_state'], samples['action'], samples['av_action'],
    ...
    def train_agent(self, samples):
        ...
        actions, av_actions, old_policy, agent_imag_feat, global_imag_feat, returns = actor_rollout(samples['observation'], samples['global_state'],
                                                                            samples['action'],
        ...
        for epoch in range(self.config.PPO_EPOCHS):
            ...
            for i in range(0, len(inds), step):
                self.cur_update += 1
                idx = inds[i:i + step]
                loss = actor_loss(agent_imag_feat[idx], actions[idx], av_actions[idx] if av_actions is not None else None,
